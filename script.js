...
(function () {
  ...
  function removeOldMobileMenu() {
    document.querySelectorAll(".mobile-menu-toggle, .mobile-menu-panel, .mobile-menu-backdrop, .mobile-menu-toggle-fixed, .mobile-menu-panel-fixed, .mobile-menu-backdrop-fixed, .mobile-menu-brand-fixed").forEach((el) => {
      el.remove();
    });

    document.body.classList.remove("mobile-menu-lock", "mobile-menu-fixed-lock", "mobile-menu-fixed-open");
    const nav = document.querySelector("nav");
  ...
  function initFixedMobileMenu() {
    ...
    brand.className = "mobile-menu-brand-fixed";
    brand.href = "/";
    brand.textContent = "WebNaMíru.online";
    brand.setAttribute("aria-label", "WebNaMíru.online - domů");

    const toggle = document.createElement("button");
    toggle.type = "button";
    toggle.className = "mobile-menu-toggle-fixed";
    toggle.setAttribute("aria-label", "Otevřít menu");
    toggle.setAttribute("aria-expanded", "false");
    toggle.innerHTML = "<span></span><span></span><span></span>";

    const panel = document.createElement("div");
    panel.className = "mobile-menu-panel-fixed";
    panel.setAttribute("aria-hidden", "true");
    ...
    const links = [
      ...
      ["Ukázky", "/ukazky"],
      ["Kontakt", "/#contact"],
      ["Nezávazný dotaz", "/#contact", "mobile-menu-panel-fixed__cta"]
    ];

    panel.innerHTML = links.map(([label, href, className]) => {
      return `<a href="${normalizeUrl(href)}" class="${className || ""}">${label}<span aria-hidden="true">→</span></a>`;
    }).join("");

    const backdrop = document.createElement("div");
    backdrop.className = "mobile-menu-backdrop-fixed";
    ...
    function setOpen(open) {
      document.body.classList.toggle("mobile-menu-fixed-open", open);
      document.body.classList.toggle("mobile-menu-fixed-lock", open);
      toggle.setAttribute("aria-expanded", open ? "true" : "false");
      toggle.setAttribute("aria-label", open ? "Zavřít menu" : "Otevřít menu");
      panel.setAttribute("aria-hidden", open ? "false" : "true");
    }
    ...
    toggle.addEventListener("click", function (event) {
      ...
      setOpen(!document.body.classList.contains("mobile-menu-fixed-open"));
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initFixedMobileMenu);
  } else {
    initFixedMobileMenu();
  }
})();
